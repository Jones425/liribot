...
var spotify = new spotifyWebApi(_spotify);
...
function run(action, item) {
	switch (action) {
		case 'concert-this':
			...
			get(
				'https://rest.bandsintown.com/artists/' +
					item +
					'/events?date=2019-01-01,2019-02-28&app_id=codingbootcamp'
			).then(function(res) {
		...
		case 'do-what-it-says':
			readFile('random.txt', 'utf8', function(err, res) {
				if (err) {
					console.log('Error occurred: ' + err);
				} else {
